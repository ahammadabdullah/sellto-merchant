...
import { Button as SidebarButton } from "@/components/dashboard/SidebarButton";
...
const navItems = [
  ...
  { icon: PackageSearch, label: "Products", link: "/dashboard/products" },
  { icon: ShoppingBasket, label: "Orders", link: "/dashboard/orders" },
  { icon: Ticket, label: "Tickets", link: "/dashboard/tickets" },
  { icon: Palette, label: "Customization", link: "/dashboard/customization" },
];
...
export default function LandingNav({ className }: { className?: string }) {
  ...
  return (
    <div className={cn("sidebar__wrapper h-screen ", className)}>
      <div className="top_wrap flex p-2">
        ...
        <Link
          href={"/dashboard"}
          className="ml-2"
          onClick={() => setActivePage(navItems[0].link)}
        >
          <svg
              ...
              <Link href={item.link} className="sidebar-link">
                ...
                >
                  <item.icon
                    ...
                    className={cn(
                      "mr-2",
                      activePage === item.link ? "text-primary2 opacity-1" : ""
                    )}
                  />
                  {item.label}
                </SidebarButton>
